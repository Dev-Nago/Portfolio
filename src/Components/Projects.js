...
function Projects() {
  ...
  return (
    <div className='bg-black text-white place-content-evenly p-5'>
          ...
          {projects.map( (project, key ) => ( 
            <div key={key} className='bg-orange-400 rounded-xl m-5 flex flex-col'>
              ...
              <div className='flex flex-row p-2 m-3'>
                <a href={project.link} className='bg-white rounded-lg text-orange-400 p-2 m-2 w-1/2'>Visite du Site</a>
                <a href={project.github} target='_blank' rel='noreferrer' className='bg-white rounded-lg text-orange-400 p-2 m-2 w-1/2'>Github</a>
              </div>
            </div>
          ))}
        </div>
